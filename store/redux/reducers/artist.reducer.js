import {
  ...
  SET_ARTIST_DEMOGRAPHICS,
  SET_SONG_DEMOGRAPHICS,
} from '../actions/types';
...
const artistReducer = (state = initialState, action) => {
  switch (action.type) {
    ...
    case SET_ARTIST_DEMOGRAPHICS:
      if (state.artist.demographics) state.artist.demographics.set_data(action.payload);
      else state.artist.demographics = new Demographic(action.payload);
      return {
        ...state,
      };
    case SET_SONG_DEMOGRAPHICS:
      ...
      if (action.payload.song instanceof Song) {
        ...
        const song_objects = state.artist.songs.filter(
          (_song) => song.song_name === _song.song_name,
        );
